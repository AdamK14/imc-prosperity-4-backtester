...
from prosperity4bt.file_reader import FileReader, FileSystemReader
...
def parse_data(data_root: Optional[Path]) -> FileReader:
    if data_root is not None:
        ...
        return FileSystemReader(Path(__file__).parent / "resources")
...
def cli(
    ...
    data: Annotated[Optional[Path], Option(help="Path to data directory (defaults to bundled resources).", show_default=False, exists=True, file_okay=False, dir_okay=True, resolve_path=True)] = None,
